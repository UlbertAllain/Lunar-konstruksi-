...
export function FormworkProjects({ data }: { data: SiteData }) {
  ...
  return (
    <div className="overflow-hidden bg-[#f5f1e8] text-[#182d4d] [font-family:'Aptos','Segoe_UI_Variable_Text','Segoe_UI',Arial,sans-serif]">
      <FormworkHeader />
      <main>
        <section className="relative border-b border-[#d9d4ca] py-16 sm:py-24"><BlueprintLayer /><div className="relative mx-auto grid w-full max-w-[1480px] gap-10 px-5 sm:px-8 lg:grid-cols-[.75fr_1.25fr] lg:px-10"><div><MicroLabel>Work / documented projects</MicroLabel><h1 className={`${displayFont} mt-8 text-[clamp(3.8rem,7vw,7.8rem)] font-black uppercase leading-[.84] tracking-[-.055em]`}>Pekerjaan nyata membentuk arsip kami.</h1><p className="mt-8 max-w-lg text-[15px] leading-7 text-[#566476]">Setiap entri berasal dari Project yang dikelola melalui admin dan menggunakan media proyek yang tersimpan di database.</p></div><div className="relative min-h-[520px]"><div className="absolute right-0 top-0 w-[86%] overflow-hidden [border-radius:58%_42%_45%_55%/40%_48%_52%_60%]"><DatabaseImage src={LOCAL_MEDIA.projectsHero || hero?.image || ""} alt={hero?.title ?? "Project"} className="h-[470px] w-full object-cover" /></div><TechnicalArc className="bottom-[-10%] left-[12%] h-[360px] w-[500px] rotate-[17deg]" /></div></div></section>
