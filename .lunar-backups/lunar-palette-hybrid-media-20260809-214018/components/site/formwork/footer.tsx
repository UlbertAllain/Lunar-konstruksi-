...
export function FormworkFooter() {
  ...
  return (
    <footer className="bg-[#162023] text-[#f4efe7]">
      <div className="mx-auto w-full max-w-[1480px] px-5 py-12 sm:px-8 lg:px-10">
        <div className="grid gap-10 border-b border-white/15 pb-10 lg:grid-cols-[1.15fr_.85fr]">
          <div>
            <p className={`${displayFont} text-4xl font-black uppercase leading-[.9] tracking-[-0.035em] sm:text-6xl`}>Lunar <span className="text-[#e36c2f]">/</span> Konstruksi</p>
          ...
          <div className="grid gap-6 text-sm sm:grid-cols-2">
            <div><p className="font-mono text-[9px] uppercase tracking-[.18em] text-[#e36c2f]">Contact</p><p className="mt-3 text-white/75">{email}</p><p className="mt-1 text-white/75">{phone}</p></div>
            <div><p className="font-mono text-[9px] uppercase tracking-[.18em] text-[#e36c2f]">Navigate</p><div className="mt-3 flex flex-col gap-2 text-white/75"><Link href="/about">About</Link><Link href="/projects">Work</Link><Link href="/services">Capabilities</Link><Link href="/contact">Contact</Link></div></div>
          </div>
        </div>
