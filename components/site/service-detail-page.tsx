...
export default function ServiceDetailPage() {
  ...
  const service =
    data.services.find((item) => item.slug === params.slug) ?? data.services[0];
      ...
      <main>
        <section className="site-section border-b border-slate-300">
          <div className="site-container grid gap-12 lg:grid-cols-[1fr_0.9fr] lg:items-center">
            <div>
              ...
              <h1 className="mt-6 text-5xl font-semibold leading-[0.93] tracking-[-0.055em] sm:text-7xl">
                {service.name}
              </h1>
              <p className="mt-8 max-w-2xl text-lg leading-8 text-slate-600">
                {service.shortDescription}
              </p>
              <Link href="/contact" className="site-button-dark mt-8">
                Konsultasikan Kebutuhan <ArrowRight size={17} />
              </Link>
            </div>
        ...
        <section className="site-section">
          <div className="site-container grid gap-12 lg:grid-cols-[0.65fr_1.35fr]">
            <div>
              <span className="site-kicker">Approach</span>
              <h2 className="mt-5 text-3xl font-semibold tracking-tight">
                Bekerja dari kebutuhan menuju detail yang dapat dibangun.
              </h2>
            </div>
            <div>
              <p className="whitespace-pre-line text-xl leading-9 text-slate-700">
                {service.description}
              </p>
        ...
        <section className="bg-[#15181e] py-20 text-white sm:py-24">
          <div className="site-container grid gap-12 lg:grid-cols-[0.65fr_1.35fr]">
            <div>
              <span className="site-kicker text-orange-400">Scope</span>
              <h2 className="mt-5 text-3xl font-semibold tracking-tight">
                Lingkup yang dapat disesuaikan dengan project.
              </h2>
            </div>
            <div className="grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-2">
              {service.scopes.map((scope, index) => (
                <div
                  ...
                  className="flex items-center gap-3 bg-[#15181e] p-5 text-sm text-slate-300"
                >
                  <Check size={17} className="text-orange-400" /> {scope.name}
                </div>
              ))}
            </div>
          </div>
