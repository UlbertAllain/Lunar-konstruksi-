...
export function FormworkProjects({ data }: { data: SiteData }) {
  ...
  return (
    <div className="overflow-hidden bg-[#f5f1e8] text-[#182d4d] [font-family:'Aptos','Segoe_UI_Variable_Text','Segoe_UI',Arial,sans-serif]">
      <FormworkHeader />
      <main>
        <section className="relative border-b border-[#d9d4ca] py-16 sm:py-24"><BlueprintLayer /><div className="relative mx-auto grid w-full max-w-[1480px] gap-10 px-5 sm:px-8 lg:grid-cols-[.75fr_1.25fr] lg:px-10"><div><MicroLabel>P-01 / Work / documented projects</MicroLabel><h1 className={`${displayFont} mt-8 text-[clamp(3.8rem,7vw,7.8rem)] font-black uppercase leading-[.84] tracking-[-.055em]`}>Pekerjaan nyata membentuk arsip kami.</h1><p className="mt-8 max-w-lg text-[15px] leading-7 text-[#566476]">Setiap entri berasal dari Project yang dikelola melalui admin dan menggunakan media proyek yang tersimpan di database.</p></div><div className="relative min-h-[520px]"><div className="absolute inset-y-[2%] right-[-2%] w-[98%] overflow-hidden border border-[#d8d1c6]/60 bg-[#f5f1e8] shadow-[0_22px_60px_rgba(20,36,63,0.10)] [clip-path:polygon(9%_0%,84%_0%,100%_10%,100%_74%,92%_100%,27%_100%,14%_93%,0%_79%,0%_17%)]"><DatabaseImage src={LOCAL_MEDIA.projectsHero || hero?.image || ""} alt={hero?.title ?? "Project"} className="h-full min-h-[500px] w-full object-cover object-center" /></div>{/* FLOATING-PROJECTS-HERO-CARD */}
