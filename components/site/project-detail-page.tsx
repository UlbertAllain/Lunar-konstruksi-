...
export default function ProjectDetailPage() {
  ...
  const project =
    data.projects.find((item) => item.slug === params.slug) ?? data.projects[0];
          ...
          <div className="site-container">
            ...
            <div className="mt-10 grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
              <div>
                ...
                <h1 className="mt-5 text-5xl font-semibold leading-[0.92] tracking-[-0.055em] sm:text-7xl">
                  {project.title}
                </h1>
              </div>
          ...
          <div className="site-container grid gap-12 lg:grid-cols-[0.7fr_1.3fr]">
            <aside className="space-y-6">
              {[
                ...
                <div key={label} className="border-b border-slate-300 pb-4">
                  <p className="site-kicker text-slate-400">{label}</p>
            ...
            <article>
              ...
              <p className="mt-6 whitespace-pre-line text-xl leading-9 text-slate-700 sm:text-2xl sm:leading-10">
                {project.description}
              </p>
              <div className="mt-12 grid gap-8 md:grid-cols-2">
                <div>
                  <h2 className="text-lg font-semibold">Lingkup pekerjaan</h2>
                  <div className="mt-5 space-y-3">
                    {project.scope.map((item) => (
                      ...
                      >
                        <Check size={16} className="text-orange-600" /> {item}
                      </p>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="mt-5 space-y-3">
                    {project.materials.map((item) => (
                      <p
                        ...
                        className="flex items-center gap-3 text-sm text-slate-600"
                      >
                        <MapPin size={16} className="text-orange-600" /> {item}
                      </p>
                    ))}
                  </div>
                </div>
              ...
              {project.gallery.map((image, index) => (
                ...
                >
                  ...
                  <img
                    ...
                    className={`w-full rounded-[24px] object-cover ${index % 3 === 0 ? "h-[620px]" : "h-[420px]"}`}
                  />
                </div>
              ))}
            </div>
